...
export function buildPhases(s) {
    var phases = {
        split:   { name: '问题拆分',   minutes: s.split,  icon: '🧩', card: 'split',  round: 0, hint: '拆分阶段：把大问题变成"今天能完成的、可判断标准"的小任务' },
        exec1:   { name: '执行轮次1',  minutes: s.exec,   icon: '⚡', card: 'exec',   round: 1, hint: '执行阶段：只做拆分好的任务，不想别的' },
        review1: { name: '复盘轮次1',  minutes: s.review, icon: '🔍', card: 'review', round: 1, hint: '复盘阶段：是否达到完成标准？原因是什么？' }
    };
    if (s.rounds >= 2) {
        phases.exec2   = { name: '执行轮次2',  minutes: s.exec,   icon: '⚡', card: 'exec',   round: 2, hint: '第二轮执行：继续深入或调整方向' };
        phases.review2 = { name: '复盘轮次2',  minutes: s.review, icon: '🔍', card: 'review', round: 2, hint: '最终复盘：输出主结论/概念卡/证据表/最小行动' };
    }
    return phases;
}
