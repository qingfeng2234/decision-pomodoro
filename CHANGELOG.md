...
## 2026-06-23：v3.6 CSS 设计令牌地基
### 改动
- style.css 抽出 :root 设计令牌（颜色/圆角/阴影/过渡），逐像素不变，为深色模式铺路

## 2026-06-23：v3.5 计时准确性 + 卡住落实 + 桌面通知

### 修复
- **后台/息屏计时漂移**：倒计时从「每秒自减」改为「基于绝对截止时刻反算」。
  切到后台标签或息屏导致 `setInterval` 被节流/暂停时，回到前台会立刻校正到真实剩余时间。
  并监听 `visibilitychange`，恢复可见时即时刷新显示。
- **桌面通知形同虚设**：权限申请从「页面加载时自动请求」（常被浏览器静默忽略）改为
  在点击「开始」（真实用户手势）时请求；权限被拦截时给出一次性提示，告知如何开启。

### 改动
- **「卡住了」策略落实为具体文字**：选择应对策略后追问一句（最小版本是什么 / 新问法是什么 /
  第一个动作是什么），把回答拼成完整一句写入当轮「卡住记录」，不再只塞一个静态标签。

### 文件变化
- `src/timer.js`：
  - 新增 `phaseDeadline` 时间戳锚点 + `tick()`；`startTimer` 用截止时刻驱动倒计时
  - 新增 `visibilitychange` 监听器，回前台即时校正
  - 新增 `ensureNotificationPermission()`，在「开始」时按用户手势申请通知权限
  - `sendNotification()` 在无权限时给出一次性开启提示
  - `handleStuckOption()` 改为追问式具体化
